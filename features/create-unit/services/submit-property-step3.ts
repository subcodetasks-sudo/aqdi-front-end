...
export type SubmitPropertyStep3Payload = {
  propertyId: number;
  units: UnitDataState[];
  contractType?: PropertyContractType;
};

export async function submitPropertyStep3({
  propertyId,
  units,
  contractType,
}: SubmitPropertyStep3Payload) {
  const response = await apiRequest<RealEstateStep3ApiResponse>(
    "/realstate/step3",
    {
      method: "POST",
      body: JSON.stringify(
        buildRealEstateStep3Payload(propertyId, units, contractType),
      ),
      cache: "no-store",
    },
  );
